...
export default function PortfolioPage() {
  ...
  async function handleDelete(url: string) {
    try {
      await apiFetch('/api/models/me/portfolio', { method: 'DELETE', body: { url } });
      setState((prev) => ({
  ...
  async function handleSetCover(url: string) {
    try {
      const updated = await apiFetch<any>('/api/models/me', {
        method: 'PUT',
        body: { coverImage: url },
      });
...
function SortableImage({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="relative group aspect-square rounded-2xl overflow-hidden bg-muted border border-border"
    >
      ...
      <div
        {...attributes}
        {...listeners}
        className="absolute top-2 right-2 bg-black/50 text-white rounded-lg p-1 opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing"
      >
